...
function generateKeystore(privateKey, password) {
    try {
        if (privateKey === undefined || password === undefined) {
            console.log(
                'usage: npm generate:keystore YourPrivateKey YourPassword'
            );
            return;
        }
        ...
        botWallet.encrypt(password).then((resolve, reject) => {
            ...
            console.log(resolve);
            console.log('\n--------- key end ---------\n');
            const verification = ethers.Wallet.fromEncryptedJsonSync(
                resolve,
                password
            );
            console.log(
                `verification - load keystore and check the address: ${verification.address}`
            );
        });
    } catch (e) {
        console.log(e);
