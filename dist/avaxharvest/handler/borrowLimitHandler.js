...
const { getRouter, getWavax, getCrToken, getJoeToken, } = require('../contract/avaxAllContracts');
const { getConfig } = require('../../dist/common/configUtil');
const { sendTransaction } = require('../common/avaxChainUtil');
const { forceCloseMessage, updateLimitMessage, } = require('../../dist/discordMessage/avaxMessage');
...
async function getSafeAvaxAmount(ahStrategy, blockTag) {
    ...
    if (openPositionId > 0) {
        ...
        logger.info(`getSafeAvaxAmount ${positionData.wantOpen[0]} ${positionData.wantOpen[1]} `);
    ...
    logger.info(`avaxBorrowCap ${avaxBorrowCap} diff ${diff} borrowed ${borrowed} safeAvaxAmount ${safeAvaxAmount} `);
    return {
        safeAvaxAmount,
        isOverBorrowed
    };
...
async function setBorrowLimit(vault) {
    try {
        const { stableCoin, ahStrategy, vaultAdaptorMK2, vaultName, decimals } = vault;
        ...
        if (safeAvax.gte(AH_CREDIT)) {
            ...
            if (safeAvaxOneMinAgo.gte(AH_CREDIT)) {
                logger.info(`No need update borrowLimit. available ${available} availableOneMinAgo ${availableOneMinAgo} AH_CREDIT ${AH_CREDIT}`);
                return;
            }
        }
        if (safeAvax.lt(AH_CREDIT)) {
            ...
            logger.info(`Borrow limit change ${current} -> ${want} diff ${diff}`);
            //(
            ...
            // )
            //)
            logger.info(`whether need update the borrowlimit: diff ${diff.gt(updateSensitivityThreshold.mul(decimals))} and (isOverBorrowed ${isOverBorrowed} or ${vaultAssets.gt(current)})`);
            if (!(diff.gt(updateSensitivityThreshold.mul(decimals)) && (isOverBorrowed || vaultAssets.gt(current)))) {
                logger.info(`No need update borrowLimit. No big borrow limit change ${current} -> ${want}`);
                return;
            }
        }
        const newBorrowLimitWant = newBorrowLimit.mul(checkResult[1]).div(E18);
        logger.info(`newBorrowLimit ${newBorrowLimit} newBorrowLimitWant ${newBorrowLimitWant}`);
...
async function getPositionOpenEvents(ahStrategy, endBlock, positionId) {
    ...
    const logs = await getEvents(filter, ahStrategy.interface, ahStrategy.provider);
...
function getRandomDuration() {
    return (Math.random() * (FORCE_CLOSE_RANGE[1] - FORCE_CLOSE_RANGE[0]) +
        FORCE_CLOSE_RANGE[0]);
}
async function forceClose(vault) {
    try {
        ...
        if (openPositionId > 0) {
            ...
            const openPositionDataEvent = await getPositionOpenEvents(ahStrategy, latestBlock.number - 1, openPositionId);
            // console.log(
            ...
            const positionDuration = latestBlock.timestamp -
                openPositionDataEvent[openPositionId].timestamp;
            logger.info(`forceClose tolerance ${vaultName}  ${tolerance} positionDuration ${positionDuration}`);
            if (positionDuration < tolerance) {
                logger.info(`No need force close  ${vaultName} because position duration ${positionDuration} less than ${tolerance}`);
                return;
            }
            const tokens = [];
            const minAmounts = [];
            if (vaultName === 'DAI yVault') {
                ...
                const stableCoinToWavax = await router.getAmountsOut(decimals, path1);
                ...
                logger.info(`amm check ${vaultName} ${stableCoinToWavax[1]} ${joeToStableCoin[1]} ${minAmounts}`);
            }
            else {
                const path1 = [wavax.address, stableCoin.address];
                const path2 = [joeToken.address, stableCoin.address];
                const wavaxToStableCoin = await router.getAmountsOut(E18, path1);
                const joeToStableCoin = await router.getAmountsOut(E18, path2);
                ...
                minAmounts[1] = joeToStableCoin[1]
                    ...
                    .div(BigNumber.from(1000));
                tokens[1] = joeToken.address;
                logger.info(`amm check ${vaultName} ${wavaxToStableCoin[1]} ${joeToStableCoin[1]} ${minAmounts}`);
            }
