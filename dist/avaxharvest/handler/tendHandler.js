...
const { getRouter, getWavax, getAvaxAggregator, } = require('../contract/avaxAllContracts');
...
async function tend(vault) {
    try {
        ...
        logger.info(`gasPrice ${gasPrice}, callCostWithPrice: ${callCostWithPrice}`);
        ...
        if (tendTrigger) {
            ...
            if (volatilityCheck) {
                logger.info(`start volatilityCheck ${vaultName} ${volatilityCheck}`);
                const stableCoinDecimal = await stableCoin.decimals();
                logger.info(`stableCoinDecimal ${vaultName} ${stableCoinDecimal}`);
                const avaxPriceInChainlink = await avaxAggregator.latestAnswer(blockTag);
                logger.info(`avaxPriceInChainlink  ${vaultName} ${avaxPriceInChainlink}`);
