...
const { getWalletNonceManager, getAvaxRpcProvider, } = require('../common/avaxChainUtil');
...
function initVaults() {
    ...
    for (let i = 0; i < vaultsConfig.length; i += 1) {
        const { vault_adaptor: vaultAdaptorAddress, strategy, stable_coin: stableCoinAddress, gas_cost: gasCost, wallet_key: walletKey, vault_name: vaultName, strategy_name: strategyName, decimals, } = vaultsConfig[i];
        console.log(`${vaultAdaptorAddress}, ${strategy}, ${gasCost}, ${stableCoinAddress} ${walletKey} ${vaultName} ${strategyName}`);
        const nonceManager = getWalletNonceManager(providerKey, walletKey);
        const vaultAdaptorMK2 = new ethers.Contract(vaultAdaptorAddress, vaultAdaptorABI, nonceManager);
        console.log(`vaultAdaptorMK2 ${vaultAdaptorMK2.address}`);
        const stableCoin = new ethers.Contract(stableCoinAddress, erc20ABI, nonceManager);
        console.log(`stableCoin ${stableCoin.address}`);
        let sABI = strategyABI;
        if (i == 0) {
            sABI = strategyDaiABI;
        }
        const ahStrategy = new ethers.Contract(strategy, sABI, nonceManager);
...
function initAvaxAggregator() {
    ...
    avaxAggregator = new ethers.Contract(aggregatorConfig, chainlinkABI, provider);
