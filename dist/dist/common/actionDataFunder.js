...
async function pretreatReceipt(messageType, transactionHash, transactionReceipt, providerKey) {
    if (!transactionReceipt) {
        ...
        transactionReceipt = await provider
            ...
            .catch((error) => {
            logger.error(error);
            throw new BlockChainCallError(`Get transaction receipt by hash: ${transactionHash}`, messageType);
        });
    }
    return transactionReceipt;
}
async function getPnlKeyData(transactionHash, transactionReceipt, providerKey) {
    transactionReceipt = await pretreatReceipt(MESSAGE_TYPES.pnl, transactionHash, transactionReceipt, providerKey);
    let result = [];
    if (transactionReceipt) {
...
async function getInvestKeyData(transactionHash, stableCoins, transactionReceipt, providerKey) {
    ...
    const tempResult = {};
    transactionReceipt = await pretreatReceipt(MESSAGE_TYPES.invest, transactionHash, transactionReceipt, providerKey);
    if (transactionReceipt) {
        const { logs } = transactionReceipt;
...
async function getHarvestKeyData(transactionHash, transactionReceipt, providerKey) {
    transactionReceipt = await pretreatReceipt(MESSAGE_TYPES.harvest, transactionHash, transactionReceipt, providerKey);
    let result = [];
    if (transactionReceipt) {
        const { logs } = transactionReceipt;
...
async function getRebalanceKeyData(transactionHash, transactionReceipt, providerKey) {
    ...
    if (transactionReceipt) {
        ...
        const systemState = await getInsurance(providerKey)
            ...
            .catch((error) => {
            logger.error(error);
            throw new BlockChainCallError("Get system's state failed", MESSAGE_TYPES.rebalance);
        });
        const exposureState = await getExposure(providerKey)
            .calcRiskExposure(systemState, { blockTag: blockNumber })
            .catch((error) => {
            logger.error(error);
            throw new BlockChainCallError("Get system's exposure state failed", MESSAGE_TYPES.rebalance);
        });
...
async function getMintOrBurnGToken(isPWRD, transactionHash, transactionReceipt, providerKey) {
    transactionReceipt = await pretreatReceipt(MESSAGE_TYPES.miniStatsPersonal, transactionHash, transactionReceipt, providerKey);
    if (transactionReceipt) {
        const { logs } = transactionReceipt;
        let gtoken = getLatestContractsAddress()[ContractNames.powerD].address.toLowerCase();
        const eventFragment = getEventFragment(erc20ABI, 'Transfer');
        if (eventFragment) {
            // logger.info(`Transfer topic: ${eventFragment.topic}`);
            if (!isPWRD) {
                gtoken =
                    getLatestContractsAddress()[ContractNames.groVault].address.toLowerCase();
            }
            for (let i = 0; i < logs.length; i += 1) {
                const { topics, address, data } = logs[i];
                if (eventFragment.topic === topics[0] &&
