...
const getPriceCheckGlobal = async () => {
    try {
        const res = await (0, queryHandler_1.query)(`select_all_protocol_pricecheck_global.sql`, []);
...
const getPriceCheckDetail = async () => {
    try {
        let result = [];
        const res = await (0, queryHandler_1.query)(`select_all_protocol_pricecheck_detailed.sql`, []);
        if (res.status !== constants_1.QUERY_ERROR) {
...
exports.getPriceCheck = getPriceCheck;
