...
const getTimestamps = async (targetTimestamp, table, filter) => {
    try {
        ...
        const [current, diff_5m, diff_1h, diff_1d, diff_1w,] = await Promise.all([
            (0, queryHandler_1.query)(`select_all_${table}.sql`, [targetTimestamp, targetTimestamp, ...filter]),
            (0, queryHandler_1.query)(`select_all_${table}.sql`, [delta.MAX_5m, delta.MIN_5m, ...filter]),
            (0, queryHandler_1.query)(`select_all_${table}.sql`, [delta.MAX_1h, delta.MIN_1h, ...filter]),
            (0, queryHandler_1.query)(`select_all_${table}.sql`, [delta.MAX_1d, delta.MIN_1d, ...filter]),
            (0, queryHandler_1.query)(`select_all_${table}.sql`, [delta.MAX_1w, delta.MIN_1w, ...filter]),
        ]);
        if (current.status === constants_1.QUERY_ERROR || diff_5m.status === constants_1.QUERY_ERROR || diff_1h.status === constants_1.QUERY_ERROR
            || diff_1d.status === constants_1.QUERY_ERROR || diff_1w.status === constants_1.QUERY_ERROR)
...
const getMaxTimestamp = async () => {
    try {
        const res = await (0, queryHandler_1.query)(`select_max_timestamp_protocol_tvl.sql`, []);
        if (res === constants_1.QUERY_ERROR)
...
const getDistincts = async (targetTimestamp, table) => {
    try {
        const res = await (0, queryHandler_1.query)(`select_distinct_${table}.sql`, [targetTimestamp, targetTimestamp]);
...
const getTVL = async (targetTimestamp) => {
    try {
        ...
        if (tvl.current) {
            return Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ "current_timestamp": tvl.current.current_timestamp, "current_date": tvl.current.current_date }, calcKPI(tvl, 'tvl_total')), calcKPI(tvl, 'tvl_pwrd')), calcKPI(tvl, 'tvl_gvt')), calcKPI(tvl, 'util_ratio')), calcKPI(tvl, 'tvl_total')), calcKPI(tvl, 'util_ratio_limit_pwrd')), calcKPI(tvl, 'util_ratio_limit_gvt'));
        }
        else {
            return {};
        }
...
const getAPY = async (targetTimestamp, productId) => {
    try {
        ...
        if (apy.current) {
            return Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ "current_timestamp": apy.current.current_timestamp, "current_date": apy.current.current_date }, calcKPI(apy, 'apy_last24h')), calcKPI(apy, 'apy_last7d')), calcKPI(apy, 'apy_daily')), calcKPI(apy, 'apy_weekly')), calcKPI(apy, 'apy_monthly')), calcKPI(apy, 'apy_all_time')), calcKPI(apy, 'apy_current'));
        }
        else {
            return {};
        }
...
const getLifeguard = async (targetTimestamp) => {
    try {
        ...
        if (lifeguard.current) {
            return Object.assign(Object.assign(Object.assign({ "current_timestamp": lifeguard.current.current_timestamp, "current_date": lifeguard.current.current_date, "name": lifeguard.current.name, "display_name": lifeguard.current.display_name }, calcKPI(lifeguard, 'amount')), calcKPI(lifeguard, 'share')), calcKPI(lifeguard, 'last3d_apy'));
        }
        else {
            return {};
        }
...
const getSystem = async (targetTimestamp) => {
    try {
        ...
        if (system.current) {
            return Object.assign(Object.assign(Object.assign(Object.assign({ "current_timestamp": system.current.current_timestamp, "current_date": system.current.current_date }, calcKPI(system, 'total_share')), calcKPI(system, 'total_amount')), calcKPI(system, 'last3d_apy')), calcKPI(system, 'hodl_bonus'));
        }
        else {
            return {};
        }
    ...
    try {
        ...
        if (stables.length > 0) {
            for (const item of stables) {
                ...
                if (stable.current) {
                    result.push(Object.assign({ "current_timestamp": stable.current.current_timestamp, "current_date": stable.current.current_date, "name": stable.current.name }, calcKPI(stable, 'amount')));
                }
                else {
                    return {};
                }
    ...
    try {
        ...
        if (vaults.length > 0) {
            for (const item of vaults) {
                ...
                if (vault.current) {
                    result.push(Object.assign(Object.assign(Object.assign({ "current_timestamp": vault.current.current_timestamp, "current_date": vault.current.current_date, "vault_name": vault.current.name, "display_name": vault.current.display_name }, calcKPI(vault, 'amount')), calcKPI(vault, 'share')), calcKPI(vault, 'last3d_apy')));
                }
                else {
                    return {};
                }
    ...
    try {
        ...
        if (reserves.length > 0) {
            for (const item of reserves) {
                const reserve = await getTimestamps(targetTimestamp, 'protocol_reserves', [item.vault_name, item.reserve_name]);
                if (reserve.current) {
                    result.push(Object.assign(Object.assign(Object.assign({ "current_timestamp": reserve.current.current_timestamp, "current_date": reserve.current.current_date, "vault_name": reserve.current.vault_name, "reserve_name": reserve.current.reserve_name, "display_name": reserve.current.display_name }, calcKPI(reserve, 'amount')), calcKPI(reserve, 'share')), calcKPI(reserve, 'last3d_apy')));
                }
                else {
                    return {};
                }
    ...
    try {
        ...
        if (strategies.length > 0) {
            for (const item of strategies) {
                const strategy = await getTimestamps(targetTimestamp, 'protocol_strategies', [item.vault_name, item.strategy_name]);
                if (strategy.current) {
                    result.push(Object.assign(Object.assign(Object.assign({ "current_timestamp": strategy.current.current_timestamp, "current_date": strategy.current.current_date, "vault_name": strategy.current.vault_name, "strategy_name": strategy.current.strategy_name, "display_name": strategy.current.display_name }, calcKPI(strategy, 'amount')), calcKPI(strategy, 'share')), calcKPI(strategy, 'last3d_apy')));
                }
                else {
                    return {};
                }
...
const getExposureStables = async (targetTimestamp) => {
    try {
        ...
        if (stables.length > 0) {
            for (const item of stables) {
                const stable = await getTimestamps(targetTimestamp, 'protocol_exposure_stables', [item.name]);
                if (stable.current) {
                    result.push(Object.assign({ "current_timestamp": stable.current.current_timestamp, "current_date": stable.current.current_date, "name": stable.current.name, "display_name": stable.current.display_name }, calcKPI(stable, 'concentration')));
                }
                else {
                    return {};
                }
    ...
    try {
        ...
        if (protocols.length > 0) {
            for (const item of protocols) {
                const protocol = await getTimestamps(targetTimestamp, 'protocol_exposure_protocols', [item.name]);
                if (protocol.current) {
                    result.push(Object.assign({ "current_timestamp": protocol.current.current_timestamp, "current_date": protocol.current.current_date, "name": protocol.current.name, "display_name": protocol.current.display_name }, calcKPI(protocol, 'concentration')));
                }
                else {
                    return {};
                }
