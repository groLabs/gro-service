...
exports.getHistoricalAPY = void 0;
const moment_1 = __importDefault(require("moment"));
const queryHandler_1 = require("./queryHandler");
const constants_1 = require("../constants");
const configUtil_1 = require("../../common/configUtil");
const launch_timestamp = (0, configUtil_1.getConfig)('blockchain.start_timestamp');
...
const parseData = async (kpi, frequency, startDate, endDate) => {
    try {
        let q;
        const fromDate = moment_1.default.unix(startDate).format('MM/DD/YYYY');
        const toDate = moment_1.default.unix(endDate).format('MM/DD/YYYY');
        switch (frequency) {
            case 'twice_daily':
        ...
        const res = await (0, queryHandler_1.query)(q, [fromDate, toDate]);
...
const getHistoricalAPY = async (_attr, _freq, _start, _end) => {
    try {
        ...
        const results = await Promise.all(attr.map((_, i) => parseData(attr[i], freq[i], start[i], end[i])));
        let parsedResults = [];
        ...
        const object = parsedResults.reduce((obj, item) => Object.assign(obj, { [item.key]: item.value }), {});
        const result = {
            "historical_stats": Object.assign({ "current_timestamp": (0, moment_1.default)().unix(), "launch_timestamp": launch_timestamp, "network": nodeEnv }, object)
        };
        return result;
    }
    catch (err) {
...
exports.getHistoricalAPY = getHistoricalAPY;
