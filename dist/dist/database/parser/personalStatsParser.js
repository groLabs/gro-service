    ...
    try {
        ...
        logs.forEach((log) => {
            ...
            const stable_amount = side === Transfer.DEPOSIT || side === Transfer.WITHDRAWAL
                ...
                : 0;
            const isGVT = ((side === Transfer.DEPOSIT || side === Transfer.WITHDRAWAL) && !log.args[2]) ||
                ...
                ? true
                : false;
            const gvt_amount = (side === Transfer.DEPOSIT || side === Transfer.WITHDRAWAL) && isGVT
                ...
                : (side === Transfer.TRANSFER_GVT_IN ||
                    ...
                    : (side === Transfer.TRANSFER_GVT_OUT ||
                        ...
                        ? -parseAmount(log.args[2], 'USD') // Transfer.value
                        : 0;
            const pwrd_amount = (side === Transfer.DEPOSIT || side === Transfer.WITHDRAWAL) && !isGVT
                ? 1 // calculated afterwards for Transfer.DEPOSIT & Transfer.WITHDRAWAL in tx
                : side === Transfer.TRANSFER_PWRD_IN
                    ? parseAmount(log.args[2], 'USD') // Transfer.value
                    : (side === Transfer.TRANSFER_PWRD_OUT ||
