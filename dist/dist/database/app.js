...
const etlPersonalStatsCache_1 = require("./etl/etlPersonalStatsCache");
...
const pgUtil_1 = require("./common/pgUtil");
const statusUtil_1 = require("./common/statusUtil");
const airdrop4handler_1 = require("./handler/airdrop4handler");
(async () => {
    try {
        ...
        if (params.length > 0) {
            switch (params[0]) {
                case 'priceCheckHDL':
                    if (params.length === 3) {
                        await (0, etlPriceCheck_1.etlPriceCheckHDL)(parseInt(params[1]), // start timestamp
                ...
                case 'groStatsHDL':
                    if (params.length === 5) {
                        await (0, etlGroStats_1.etlGroStatsHDL)(parseInt(params[1]), // start timestamp
                        parseInt(params[2]), // end timestamp
                        params[3], // attribute
                ...
                case 'personalStatsETL':
                    if (params.length === 4 && (0, globalUtil_1.checkDateRange)(params[1], params[2])) {
                        await (0, registryLoader_1.loadContractInfoFromRegistry)();
                        await (0, etlPersonalStats_1.etlPersonalStats)(params[1], // start date
                        params[2], // end date
                ...
                case 'personalStatsETLcache':
                    if (params.length === 2) {
                        await (0, registryLoader_1.loadContractInfoFromRegistry)();
                        await (0, etlPersonalStatsCache_1.etlPersonalStatsCache)(params[1]); // account
                    }
                    else {
                ...
                case 'loadTokenPrice':
                    if (params.length === 3) {
                        await (0, registryLoader_1.loadContractInfoFromRegistry)();
                        await (0, loadTokenPrice_1.loadTokenPrice)(params[1], // start date
                        params[2]); // end date
                    }
                    else {
                ...
                case 'loadBalances':
                    if (params.length === 6 && (0, globalUtil_1.checkDateRange)(params[1], params[2])) {
                        await (0, registryLoader_1.loadContractInfoFromRegistry)();
                        await (0, loadUserBalances_1.loadUserBalances)(params[1], // start date
                        params[2], // end date
                        params[3], // account
                        params[4], // time
                ...
                case 'airdrop4':
                    if (params.length === 4) {
                        await (0, registryLoader_1.loadContractInfoFromRegistry)();
                        await (0, airdrop4handler_1.airdrop4HandlerV2)(parseInt(params[1]), // start position in addr list
                ...
                case 'checkPosition':
                    if (params.length === 3) {
                        await (0, registryLoader_1.loadContractInfoFromRegistry)();
                        await (0, airdrop4handler_1.checkPosition)(params[1], // address
                        params[2]); // timestamp
                    }
                    else {
                ...
                case 'dumpTable':
                    if (params.length === 3) {
                        await (0, pgUtil_1.dumpTable)(params[1], // table name
                ...
                case 'status':
                    if (params.length === 4) {
                        await (0, registryLoader_1.loadContractInfoFromRegistry)();
                        await (0, statusUtil_1.status)(params[1], // date
                        params[2], // time
                        params[3]); // block
                    }
                    else {
