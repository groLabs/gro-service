"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = __importDefault(require("express"));
const express_validator_1 = require("express-validator");
// const { wrapAsync } = require('../common/wrap');
const error_1 = require("../../common/error");
const groStatsHandler_1 = require("../handler/groStatsHandler");
const priceCheckHandler_1 = require("../handler/priceCheckHandler");
const historicalAPY_1 = require("../handler/historicalAPY");
const personalStatsHandler_1 = require("../handler/personalStatsHandler");
const validate_1 = require("../../stats/common/validate");
const router = express_1.default.Router();
const wrapAsync = function wrapAsync(fn) {
    return function wrap(req, res, next) {
        fn(req, res, next).catch(next);
    };
};
router.get('/gro_stats', (0, validate_1.validate)([
    (0, express_validator_1.query)('network')
        .trim()
        .notEmpty()
        .withMessage(`network can't be empty.`),
]), wrapAsync(async (req, res) => {
    let { network } = req.query;
    network = network || '';
    if (network.toLowerCase() !== process.env.NODE_ENV.toLowerCase()) {
        throw new error_1.ParameterError('Parameter network failed in database.js->router.get->/gro_stats');
    }
    const groStats = await (0, groStatsHandler_1.getAllStats)();
    res.json(groStats);
}));
router.get('/personal_stats', (0, validate_1.validate)([
    (0, express_validator_1.query)('network')
        .trim()
        .notEmpty()
        .withMessage(`network can't be empty.`),
    (0, express_validator_1.query)('address')
        .notEmpty()
        .withMessage(`address can't be empty`)
        .isLength({ min: 42, max: 42 })
        .withMessage('address must be 42 characters long')
        .matches(/^0x[A-Za-z0-9]{40}/)
        .withMessage('should be a valid address and start with "0x".'),
]), wrapAsync(async (req, res) => {
    let { network, address } = req.query;
    network = network || '';
    if (network.toLowerCase() !== process.env.NODE_ENV.toLowerCase()) {
        throw new error_1.ParameterError(`Parameter network failed in database: ${network.toLowerCase()} vs. ${process.env.NODE_ENV.toLowerCase()}`);
    }
    console.log('OKI:', network, address);
    const personalStats = await (0, personalStatsHandler_1.getPersonalStats)(address);
    res.json(personalStats);
}));
router.get('/price_check', (0, validate_1.validate)([
    (0, express_validator_1.query)('network')
        .trim()
        .notEmpty()
        .withMessage(`network can't be empty.`),
]), wrapAsync(async (req, res) => {
    let { network } = req.query;
    network = network || '';
    if (network.toLowerCase() !== process.env.NODE_ENV.toLowerCase()) {
        throw new error_1.ParameterError('Parameter network failed in database.js->router.get->/gro_stats');
    }
    const priceCheck = await (0, priceCheckHandler_1.getPriceCheck)();
    res.json(priceCheck);
}));
//http://localhost:3010/database/historical_apy?network=ropsten&attr=apy_last7d,apy_last7d,apy_last7d&freq=twice_daily,daily,weekly&start=1625097600,1625097600,1625097600&end=1629936000,1629936000,1629936000
router.get('/historical_apy', (0, validate_1.validate)([
    (0, express_validator_1.query)('network')
        .trim()
        .notEmpty()
        .withMessage(`network can't be empty`),
    (0, express_validator_1.query)('attr')
        .notEmpty()
        .withMessage(`attr can't be empty`),
    (0, express_validator_1.query)('freq')
        .notEmpty()
        .withMessage(`freq can't be empty`),
    (0, express_validator_1.query)('start')
        .notEmpty()
        .withMessage(`start can't be empty`),
    (0, express_validator_1.query)('end')
        .notEmpty()
        .withMessage(`end can't be empty`),
]), wrapAsync(async (req, res) => {
    let { network, attr, freq, start, end } = req.query;
    network = network || '';
    if (network.toLowerCase() !== process.env.NODE_ENV.toLowerCase()) {
        throw new error_1.ParameterError('Parameter network failed in database.js->router.get->/historical_apy');
    }
    const groStats = await (0, historicalAPY_1.getHistoricalAPY)(attr, freq, start, end);
    res.json(groStats);
}));
// router.get(
//     '/table_dump',
//     validate([
//         query('network')
//             .trim()
//             .notEmpty()
//             .withMessage(`network can't be empty.`),
//         query('table')
//             .trim()
//             .notEmpty()
//             .withMessage(`table can't be empty.`),
//     ]),
//     wrapAsync(async (req, res) => {
//         let { network, table } = req.query;
//         network = network || '';
//         if (network.toLowerCase() !== process.env.NODE_ENV.toLowerCase()) {
//             throw new ParameterError('Parameter network failed in database.js->router.get->/gro_stats');
//         }
//         const tableDump = await dumpTable(table, false);
//         res.set('Content-Type', 'application/octet-stream');
//         res.send(tableDump);
//     })
// );
exports.default = router;
