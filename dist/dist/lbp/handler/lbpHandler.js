...
const getLbpBalanceAndPrice = async () => {
    try {
        const [balanceAndTimestamp, latest_price, price_1h] = await Promise.all([
...
const getLbpStatsDB = async () => {
    try {
        const result = {
            "lbp_stats": Object.assign({ "gro_price_history": await getLbpHistoricPrice() }, await getLbpBalanceAndPrice())
        };
        return result;
    }
    catch (err) {
