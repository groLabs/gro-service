...
const callSubgraph = async (payload) => {
    ...
    switch (payload.query) {
        case 'uniswapVolume':
            q = uniswapVolume(payload.id, payload.block);
            break;
        case 'balancerVolume':
            q = balancerVolume(payload.id, payload.addr, payload.block);
            break;
        default:
            logger.error(`Error in subgraphCaller.js->callSubgraph(): Invalid subgraph request (${payload.query})`);
            return null;
    }
    const result = await axios.post(payload.url, { query: q }).catch(err => {
        if (err.response) {
