...
const { getAlchemyRpcProvider, getTimestampByBlockNumber, } = require('../../common/chainUtil');
const { getSimpleFilterEvents, getFilterEvents, } = require('../../common/logFilter-new');
const { getConfig } = require('../../common/configUtil');
const { getContractsHistory, getLatestContractsAddressByAddress, } = require('../../registry/registryLoader');
...
async function getAssetsChangedEvents(vault, strategy, startBlock, endBlock) {
    logger.info(`getAssetsChangedEvents : startBlock: ${startBlock.blockNumber}, endBlock: ${endBlock.blockNumber}`);
    ...
    const harvestedLogs = await getSimpleFilterEvents(harvestFilter, providerKey);
    if (harvestedLogs.length < 2) {
        logger.info(`no enough harvest. only have ${harvestedLogs.length}`);
        return assetChangedBlock;
    }
    const reverseHarvestedEvents = harvestedLogs.sort((a, b) => b.blockNumber - a.blockNumber);
    ...
    const vaultTransferFilter = vault.filters.Transfer(null, '0x0000000000000000000000000000000000000000');
    vaultTransferFilter.fromBlock = startHarvest.blockNumber;
    vaultTransferFilter.toBlock = endHarvest.blockNumber;
    const vaultTransferLogs = await getSimpleFilterEvents(vaultTransferFilter, providerKey);
    ...
    for (let i = 0; i < assetChangedBlock.length; i += 1) {
        ...
        // eslint-disable-next-line no-await-in-loop
        logger.info(`assetChangedBlock[i].blockNumber: ${assetChangedBlock[i].blockNumber}`);
    ...
    const sortedBlocks = assetChangedBlock.sort((a, b) => a.blockNumber - b.blockNumber);
    return sortedBlocks;
}
async function calcStrategyAPY(vault, strategy, startBlock, endBlock, defaultApy) {
    const sortedBlocks = await getAssetsChangedEvents(vault, strategy, startBlock, endBlock);
    ...
    for (let i = 0; i < sortedBlocks.length; i += 1) {
        ...
        logger.info(`sortedBlocks ${b.blockNumber} ${b.timestamp} ${b.totalAssets} ${b.totalGain} ${b.totalLoss}`);
        if (i + 1 < sortedBlocks.length) {
            const duration = BigNumber.from(sortedBlocks[i + 1].timestamp - b.timestamp);
            totalAssets = totalAssets.add(b.totalAssets.mul(duration));
        }
    }
    const totalDuration = BigNumber.from(sortedBlocks[sortedBlocks.length - 1].timestamp -
        sortedBlocks[0].timestamp);
    const timeWeightedTotalAssets = totalAssets.div(totalDuration);
    logger.info(`TotalAssets ${totalAssets} duration ${totalDuration} timeWeightedTotalAssets ${timeWeightedTotalAssets} `);
    const totalGain = sortedBlocks[sortedBlocks.length - 1].totalGain
        .sub(sortedBlocks[0].totalGain)
        .sub(sortedBlocks[sortedBlocks.length - 1].totalLoss.sub(sortedBlocks[0].totalLoss));
    // .sub(startExpectedReturn);
    logger.info(`timeWeightedTotalAssets ${timeWeightedTotalAssets} totalGain ${totalGain} start ${sortedBlocks[0].totalGain}  end ${sortedBlocks[sortedBlocks.length - 1].totalGain}`);
    const apy = totalGain
        .mul(PERCENT_DECIMAL)
...
async function calcCurrentStrategyAPY(startBlock, endBlock) {
    logger.info(`calculate strategy apy ${startBlock.blockNumber}  ${endBlock.blockNumber}`);
    ...
    for (let i = 0; i < vaults.length; i += 1) {
        ...
        for (let j = 0; j < strategies.length; j += 1) {
            ...
            // eslint-disable-next-line no-await-in-loop
            const apy = await calcStrategyAPY(yearnVaults[i], strategy, startBlock, endBlock, BigNumber.from(metaData.DY));
...
async function getGtokenApy(systemApy, utilRatio, hodlBonus) {
    let y = BigNumber.from(300000).add(utilRatio.mul(BigNumber.from(3)).div(BigNumber.from(8)));
    // 0.6 + ( 2 * ( util_ratio - 0.8 ))
    if (utilRatio.gt(BigNumber.from(800000))) {
        y = BigNumber.from(600000).add(BigNumber.from(2).mul(utilRatio.sub(BigNumber.from(800000))));
    }
...
async function getHodlBonusApy() {
    ...
    for (let i = 0; i < pnlFilters.length; i += 1) {
        ...
        pnlFilterPromise.push(getFilterEvents(filter.filter, filter.interface, providerKey));
    }
    ...
    pnlLogs.forEach((log) => {
        logger.info(`pnlLogs ${log.blockNumber} ${log.args[4]} ${log.args[6]} ${log.args[7]} ${withdrawalBonus} ${log.args[3]} ${priceChanged}`);
        withdrawalBonus = withdrawalBonus.add(log.args[4].mul(PERCENT_DECIMAL).div(log.args[6].add(log.args[7])));
        priceChanged = priceChanged.add(log.args[3].mul(PERCENT_DECIMAL).div(log.args[6]));
    });
