...
function mapperToUsd(original, keys) {
    return mapObject(original, (key, value) => {
...
async function getCurvePoolRatio(curvePool, blockTag) {
    ...
    for (let i = 1; i < 3; i += 1) {
        const ratio = await curvePool.get_dy(0, i, STABLECION_DECIMALS[0], blockTag);
    ...
    logger.info(`curve pool ratios ${ratios.dai_usdc} ${ratios.dai_usdt} ${ratios.usdt_usdc}`);
    const usdc_usdt = await curvePool.get_dy(1, 2, STABLECION_DECIMALS[2], blockTag);
    const usdt_usdc = await curvePool.get_dy(2, 1, STABLECION_DECIMALS[2], blockTag);
...
async function getChainlinkPrice(buoyInstance, blockTag) {
    ...
    logger.info(`ratios ${ratios.dai_usdc} ${ratios.dai_usdt} ${ratios.usdt_usdc}`);
    return ratios;
}
async function getBouyLastRatio(buoyInstance, blockTag) {
    ...
    logger.info(`cache ratios ${ratios.dai_usdc} ${ratios.dai_usdt} ${ratios.usdt_usdc}`);
    return ratios;
}
...
function checkTolerance(diff, tolerance) {
    ...
    logger.info(`ref check ${check.dai_usdc} ${check.dai_usdt} ${check.usdt_usdc}`);
    return check;
}
...
async function getGroPrice(blockNumberStr) {
    ...
    const buoyInstance = getLatestSystemContract(ContractNames.buoy3Pool, providerKey).contract;
    let blockNumber;
    if (blockNumberStr === 'latest') {
    ...
    const oracleCheckTolerance = await buoyInstance.oracle_check_tolerance(blockTag);
    let curveCheckTolerance = oracleCheckTolerance;
    try {
        curveCheckTolerance = await buoyInstance.curve_check_tolerance(blockTag);
    }
    catch (e) {
        logger.info(`${e}`);
    }
    ...
    const curvePool = new ethers.Contract(curvePoolAddr, Curve3PoolABI, buoyInstance.provider);
    ...
    const curveVsCacheCheck = checkTolerance(curveVsCache, oracleCheckTolerance);
    const curveVsChainlinkCheck = checkTolerance(curveVsChainlink, oracleCheckTolerance);
