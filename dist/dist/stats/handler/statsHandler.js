...
const { getTvlStats, getSystemStats, getExposureStats, } = require('./systemHandler');
...
async function generateGroStatsFile() {
    ...
    apy.current = await getGtokenApy(system.last3d_apy, tvl.util_ratio, apy.hodl_bonus);
    stats.apy = mapper(apy, ['pwrd', 'gvt', 'hodl_bonus'], []);
    stats.tvl = mapper(tvl, ['util_ratio', 'util_ratio_limit_PD', 'util_ratio_limit_GW'], ['pwrd', 'gvt', 'total']);
    stats.system = mapper(system, ['total_share', 'share', 'last3d_apy'], ['total_amount', 'amount']);
    ...
    fs.writeFileSync(argentStatsFilename, JSON.stringify(formatArgentResponse(stats)));
    const externalStatsFilename = `${statsDir}/external-stats-${latestBlock.timestamp}.json`;
    fs.writeFileSync(externalStatsFilename, JSON.stringify(formatExternalResponse(stats)));
    ...
    logger.info(`Power Dollar:${stats.tvl.pwrd} Gro Vault:${stats.tvl.gvt} TotalAssets:${stats.tvl.total} Utilization Ratio:${stats.tvl.util_ratio}`);
    apyStatsMessage({
        vaultTVL: tvl.gvt,
