...
async function readUniswapPool(uniswapPool, tokenAUsdPrice, tokenBUsdPrice, blockTag) {
    ...
    logger.info(`tvl ${tvl}`);
    const stakedLP = await uniswapPool.balanceOf(lpTokenStaker.address, blockTag);
...
async function readUniswapPoolWithDecimal(uniswapPool, tokenAUsdPrice, tokenBUsdPrice, tokenADecimal, tokenBDecimal, blockTag) {
    const reserved = await uniswapPool.getReserves(blockTag);
    logger.info(`Uniswap reserved ${reserved[0]} ${reserved[1]}`);
    const tvl = reserved[0]
        .mul(tokenAUsdPrice)
    ...
    logger.info(`tvl ${tvl} ${tokenBUsdPrice} ${tokenBDecimal} ${reserved[1]
        .mul(tokenBUsdPrice)
        .div(tokenBDecimal)}`);
    const stakedLP = await uniswapPool.balanceOf(lpTokenStaker.address, blockTag);
    logger.info(`stakedLP ${stakedLP}`);
...
async function getGroPriceFromUniswap(blockTag) {
    ...
    const groGvtPricePair = await uniswapRoute.getAmountsOut(ONE, [groToken.address, groVault.address], blockTag);
...
//     };
// }
async function getUniswapGroGvtStats(priceOracle, currentApy, groPerBlock, totalAllocPoint, latestBlock, block24hAgo) {
    // pool-0
    const poolOneInfo = await readUniswapPool(uniswapGroGvtPool, priceOracle.gvtPriceInUsd, priceOracle.groPriceInUsd, latestBlock);
    // const tag24HoursAgo = {
    ...
    logger.info(`currentApy.gvt ${currentApy.gvt} tokenApy ${poolOneInfo.tokenApy}`);
    const poolOneStakingInfo = await lpTokenStaker.poolInfo(pool1Config.pid, latestBlock);
    logger.info(`groPerBlock ${groPerBlock} poolOneInfo ${poolOneStakingInfo.allocPoint} totalAllocPoint ${totalAllocPoint}`);
...
async function getUniswapGroUsdcStats(priceOracle, currentApy, groPerBlock, totalAllocPoint, latestBlock, block24hAgo) {
    // pool-0
    const poolOneInfo = await readUniswapPoolWithDecimal(uniswapGroUsdcPool, priceOracle.groPriceInUsd, ONE, ONE, BigNumber.from('1000000'), latestBlock);
    ...
    logger.info(`currentApy.gvt ${currentApy.gvt} tokenApy ${poolOneInfo.tokenApy}`);
    const poolOneStakingInfo = await lpTokenStaker.poolInfo(pool2Config.pid, latestBlock);
    logger.info(`groPerBlock ${groPerBlock} poolOneInfo ${poolOneInfo.allocPoint} totalAllocPoint ${totalAllocPoint}`);
    if (poolOneInfo.stakedLP.isZero()) {
        poolOneInfo.rewardApy = BigNumber.from(0);
    }
    else {
...
//     return poolOneInfo;
// }
async function getSingleGroStats(priceOracle, groPerBlock, totalAllocPoint, latestBlock) {
    ...
    const poolTwoInfo = await lpTokenStaker.poolInfo(pool0Config.pid, latestBlock);
...
async function getSingleGvtStats(priceOracle, groPerBlock, totalAllocPoint, currentApy, latestBlock) {
    ...
    logger.info(`totalGroTvl single tvl ${totalGroTvl}`);
    const poolTwoInfo = await lpTokenStaker.poolInfo(pool3Config.pid, latestBlock);
    let rewardApy = BigNumber.from(0);
    if (!singleStaked.isZero()) {
        rewardApy = groPerBlock
            .mul(priceOracle.groPriceInUsd)
...
async function getCurveLpApy(curvePool, latestBlockTag, startBlock) {
    const currentVirtualPrice = await curvePool.get_virtual_price(latestBlockTag);
    ...
    if (startBlock && startBlock > blockNumber24Hours) {
        logger.info(`startBlock ${startBlock} > blockNumber24Hours ${blockNumber24Hours}`);
...
async function calculateCurveMetaPoolApy(priceOracle, currentApy, latestBlock) {
    ...
    // pool coins balance
    const metaPool3CrvBalance = await curve3CrvLpToken.balanceOf(curvePwrd3crvPool.address);
    const metaPoolUPwrdBalance = await groPwrd.balanceOf(curvePwrd3crvPool.address);
    logger.info(`metaPool3CrvBalance balance : ${metaPool3CrvBalance} metaPoolUPwrdBalance ${metaPoolUPwrdBalance}`);
    ...
    let poolApy = await getCurveLpApy(curvePwrd3crvPool, latestBlock, metaPoolFirstAddLiquidityBlock);
    ...
    const stakedAmount = await groPwrdUsdcLpToken.balanceOf(lpTokenStaker.address);
    // lp price
    const curvePwrd3crvLpPrice = await curvePwrd3crvPool.calc_withdraw_one_coin(ONE, 0, latestBlock);
    ...
    const poolGroYear = groPerBlock
        ...
        .div(totalAllocPoint);
    logger.info(`groPerBlock: ${groPerBlock} priceOracle.groPriceInUsd ${priceOracle.groPriceInUsd} totalAllocPoint: ${totalAllocPoint} poolAllocPoint: ${poolInfo.allocPoint} poolGroYear: ${poolGroYear} ${stakedAmount}`);
    ...
    // TVL
    const pwrd3crvLpTotalSupply = await groPwrdUsdcLpToken.totalSupply(latestBlock);
    ...
    logger.info(`staked amount : ${stakedAmount} unstaked amount : ${unstaked}`);
...
const calcFees = async (pool, block24hAgo) => {
    try {
        ...
        const isUniswap = pool === 'uniswap_v2_5050_gro_gvt_1' ||
        ...
        const poolId = pool === 'uniswap_v2_5050_gro_gvt_1'
            ? sgGvtGroPoolId
            : pool === 'uniswap_v2_5050_gro_usdc_2'
                ? sgGroUsdcPoolId
                : pool === 'balancer_v2_8020_gro_weth_5'
                    ...
                    : -1;
        if (poolId === -1) {
            logger.error(`Error in groTokenHandler.js->calcFees(): Unknown pool`);
            return ZERO;
        }
        const payloadNow = {
            query: query,
        ...
        if (isUniswap) {
            ...
            logger.info(`Fees calc => vol now: ${vNow}, vol 24h: ${v24h}, vol diff: ${vNow - v24h}, vol incl. fee: ${swapFees24h}`);
            ...
            logger.info(`Fees calc => feeVol now: ${vNow}, feeVol 24h: ${v24h}, vol diff: ${vNow - v24h}`);
            return floatToBN(swapFees24h);
        }
    }
    catch (err) {
...
const getBalancerGroWethStats = async (priceOracle, groPerBlock, totalAllocPoint, block24hAgo) => {
    try {
        let pools;
        let poolShares;
        ...
        const poolFiveStakingInfo = await lpTokenStaker.poolInfo(pool5Config.pid, null);
...
async function getPools(currentApy, latestBlock) {
    ...
    logger.info(`groPerBlock ${groPerBlock} totalAllocPoint ${totalAllocPoint}`);
    ...
    logger.info(' -- pool0');
    const poolSingleGroStats = await getSingleGroStats(priceOracle, groPerBlock, totalAllocPoint, latestBlock);
    logger.info(' -- pool1');
    const poolUniswapGroGvtStats = await getUniswapGroGvtStats(priceOracle, currentApy, groPerBlock, totalAllocPoint, latestBlock, block24hAgo);
    logger.info(' -- pool2');
    const poolUniswapGroUsdcStats = await getUniswapGroUsdcStats(priceOracle, currentApy, groPerBlock, totalAllocPoint, latestBlock, block24hAgo);
    logger.info(' -- pool3');
    const poolSingleGvtStats = await getSingleGvtStats(priceOracle, groPerBlock, totalAllocPoint, currentApy, latestBlock);
    logger.info(' -- pool4');
    const curveMetaStats = await calculateCurveMetaPoolApy(priceOracle, currentApy, latestBlock);
    logger.info(' -- pool5');
    const poolBalancerGroWethStats = await getBalancerGroWethStats(priceOracle, groPerBlock, totalAllocPoint, block24hAgo);
    const pools = [
        {
        ...
        },
        {
            ...
            apy: {
                current: {
                    ...
                    pool_incentive: isNaN(poolBalancerGroWethStats.poolIncentive)
                        ? NAH
...
async function getLpTokenAmountOfBalancerPool(address) {
    const balancer = await getLatestSystemContract(ContractNames.BalancerWeightedPool).contract;
