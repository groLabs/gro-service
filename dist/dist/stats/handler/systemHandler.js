...
async function getUsdValue(i, amount, blockTag) {
    const usdValue = getLatestSystemContract(ContractNames.buoy3Pool).contract.singleStableToUsd(amount, i, blockTag);
    return usdValue;
}
async function getUsdValueForLP(amount, blockTag) {
    const usdValue = getLatestSystemContract(ContractNames.buoy3Pool).contract.lpToUsd(amount, blockTag);
    return usdValue;
}
    ...
    try {
        ...
        for (let vaultIndex = 0; vaultIndex < vaults.length - 1; vaultIndex += 1) {
            ...
            for (let strategyIndex = 0; strategyIndex < strategies.length; strategyIndex += 1) {
                ...
                const strategyStatus = await yearnVaults[vaultIndex].strategies(strat.address, blockTag);
                const totalAssets = strategyStatus.totalDebt;
                const estimatedTotalAssets = await strat.estimatedTotalAssets(blockTag);
                const expectedReturn = estimatedTotalAssets.sub(totalAssets);
                const msg = `Strategy expected return check vaultidx ${vaultIndex} stratidx ${strategyIndex} total ${totalAssets.div(DECIMALS[vaultIndex])} estimated ${estimatedTotalAssets.div(DECIMALS[vaultIndex])} expectedReturn ${expectedReturn.div(DECIMALS[vaultIndex])}`;
                logger.info(msg);
                if (expectedReturn.lt(BigNumber.from(0)) &&
                    totalAssets.gt(BigNumber.from(0))) {
                    const diff = expectedReturn
                        .abs()
                        .mul(SHARE_DECIMAL)
                        .div(totalAssets);
                    const expectedReturnUsd = expectedReturn.div(DECIMALS[vaultIndex]);
...
async function getCurveVaultStats(blockTag) {
    const curveVault = getLatestSystemContract(ContractNames.CRVVaultAdaptor).contract;
    ...
    const strategyStats = await getCurveStrategyStats(curveVault, vaultTotalAsset, blockTag);
    return {
        name: 'Curve yVault',
        amount: assetUsd,
...
async function getStrategiesStats(vault, index, length, vaultTotalAsset, blockTag) {
    const strategies = [];
    let reservedAssets = BigNumber.from(vaultTotalAsset);
    logger.info(`vta ${vaultTotalAsset} ra ${reservedAssets}`);
    const vaultAndStrateyInfo = await getLatestVaultsAndStrategies(providerKey);
    const { contracts: vaultAndStrategies } = vaultAndStrateyInfo;
    const { contractInfo: vaultInfo, vault: yearnVault } = vaultAndStrategies[vault.address];
    ...
    for (let j = 0; j < length; j += 1) {
        ...
        reservedAssets = reservedAssets.sub(strategyAssets);
        const strategyAssetsUsd = await getUsdValue(index, strategyAssets, blockTag);
        strategies.push({
...
async function getVaultStats(blockTag) {
    ...
    for (let vaultIndex = 0; vaultIndex < vaults.length - 1; vaultIndex += 1) {
        ...
        const assetUsd = await getUsdValue(vaultIndex, vaultTotalAsset, blockTag);
        const { strategyLength, contractInfo: vaultInfo } = vaultStrategyContracts[vault.address];
        const strategyStats = await getStrategiesStats(vault, vaultIndex, strategyLength, vaultTotalAsset, blockTag);
        vaultAssets.push({
...
async function getLifeguardStats(blockTag) {
    ...
    const { contract: lifeguardContract, contractInfo: lifeguardContractInfo } = lifeGuard;
...
async function getPrepareCalculation(systemStats, blockTag) {
    const { contract: insurance } = getLatestSystemContract(ContractNames.insurance);
    const lifeguardCurrentAssetsUsd = systemStats.lifeguard.amount;
    let totalCurrentAssetsUsd = BigNumber.from(0);
    const curveCurrentAssetsUsd = systemStats.vault[systemStats.vault.length - 1].amount;
    ...
    for (let i = 0; i < systemStats.vault.length - 1; i += 1) {
        ...
        totalCurrentAssetsUsd = totalCurrentAssetsUsd.add(vaultCurrentAssetsUsd[i]);
        stablePercents[i] = await insurance.underlyingTokensPercents(i, blockTag);
    }
...
async function getExposureStats(blockTag, systemStats) {
    const { contract: exposure } = getLatestSystemContract(ContractNames.exposure);
    logger.info(`getExposureStats blockTag : ${JSON.stringify(blockTag)}`);
    const stableCoins = await getLatestStableCoins(providerKey);
    const { contract: bouy } = await getLatestSystemContract(ContractNames.buoy3Pool);
    const safetyCheck = await bouy.safetyCheck();
    let preCal;
    if (safetyCheck) {
        preCal = await getLatestSystemContract(ContractNames.insurance).contract.prepareCalculation(blockTag);
    }
    else {
        sendAlertMessage({
            discord: {
    ...
    const vaultAndStrateyInfo = await getLatestVaultsAndStrategies(providerKey);
    const { vaultsAddress: adapterAddresses, contracts: vaultStrategies } = vaultAndStrateyInfo;
    for (let i = 0; i < vaultsStats.length; i += 1) {
        ...
        for (let j = 0; j < strategyList.length; j += 1) {
            ...
            const { protocols: strategyProtocols, protocolsDisplayName } = contractInfo;
            for (let k = 0; k < strategyProtocols.length; k += 1) {
                ...
                if (index >= 0) {
                    ...
                    exposure.concentration = exposure.concentration.add(strategy.share);
                }
                else {
...
async function getTvlStats(blockTag) {
    ...
    const gvtAssets = await getLatestSystemContract(ContractNames.groVault).contract.totalAssets(blockTag);
    const prwdAssets = await getLatestSystemContract(ContractNames.powerD).contract.totalSupply(blockTag);
    const totalAssetsUsd = gvtAssets.add(prwdAssets);
    const utilRatio = calculateSharePercent(prwdAssets, gvtAssets);
    const controller = getLatestSystemContract(ContractNames.controller).contract;
    const utilRatioLimitPD = await controller.utilisationRatioLimitPwrd(blockTag);
    const utilRatioLimitGW = await controller.utilisationRatioLimitGvt(blockTag);
    const tvl = {
        pwrd: prwdAssets,
        gvt: gvtAssets,
...
async function getSystemStats(totalAssetsUsd, blockTag) {
    ...
    lifeGuardStats.share = calculateSharePercent(lifeGuardStats.amount, totalAssetsUsd);
    ...
    const vaultStats = vaultAssets.map((vaultAsset, vaultIndex) => {
        ...
        const { contractInfo: vaultAdapterInfo } = vaultAdaptersInfo[vaultAddress];
        ...
        const strategies = vaultAsset.strategies.map((strategy, strategyIndex) => {
            ...
            if (strat !== undefined) {
                logger.info(`strat apy ${strategyInfo.contract.address} ${strat.apy}`);
                stratApy = strat.apy;
            }
            const strategyPercent = calculateSharePercent(strategy.amount, totalAssetsUsd);
            logger.info(`strat address ${strategyIndex} ${stratApy} ${strategyPercent}`);
