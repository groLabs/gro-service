...
async function fetchTimestamp(transaction, provider) {
    ...
    transaction.timestamp = await getBlockNumberTimestamp(blocknumber, provider);
...
function parseData(events, token, type, transferType) {
    ...
    for (let i = 0; i < events.length; i += 1) {
        ...
        const item = {
            ...
            usd_amount: div(event.amount, CONTRACT_ASSET_DECIMAL, amountDecimal),
            coin_amount: div(event.coin_amount, CONTRACT_ASSET_DECIMAL, amountDecimal),
...
function getDepositWithdrawTransfer(groVault, powerD) {
    ...
    const groDepositTransactions = parseData(groDepositEvents, 'gvt', 'deposit', 'transfer_in');
    const groWithdrawTransactions = parseData(groWithdrawEvents, 'gvt', 'withdrawal', 'transfer_out');
    const pwrdDepositTransactions = parseData(pwrdDepositEvents, 'pwrd', 'deposit', 'transfer_in');
    const pwrdWithdrawTransactions = parseData(pwrdWithdrawEvents, 'pwrd', 'withdrawal', 'transfer_out');
    return [
...
async function getTransactions(groVault, powerD, provider) {
    const depositWithdrawTransferEvents = getDepositWithdrawTransfer(groVault, powerD);
    await appendEventTimestamp(depositWithdrawTransferEvents, provider);
    return depositWithdrawTransferEvents;
}
async function getTransaction(depositWithdrawTransferEvents, approvalEvents, failTransactions, provider) {
    ...
    for (let i = 0; i < depositWithdrawTransferEvents.length; i += 1) {
        ...
        const { token, transaction, hash, usd_amount: usdAmount, block_number: blockNumber, coin_amount: coinAmount, timestamp, } = event;
        switch (transaction) {
            case 'deposit':
    ...
    for (let i = 0; i < approvalEvents.length; i += 1) {
        const { token, hash, spender, coin_amount: coinAmount, usd_amount: usdAmount, block_number: blockNumber, timestamp, } = approvalEvents[i];
