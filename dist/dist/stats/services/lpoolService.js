...
const lptokenStaker = new ethers.Contract(LPTokenStakerInfo.address, LPTokenStakerABI, provider);
const grovesting = new ethers.Contract(GROVestingInfo.address, GROVestingABI, provider);
async function fetchStakeAndUnstakeTransactions(account, endBlock) {
    ...
    const eventPromise = [];
    eventPromise.push(getFilterEvents(stakeFilter, lptokenStaker.interface, providerKey));
    eventPromise.push(getFilterEvents(unstakeFilter, lptokenStaker.interface, providerKey));
    const events = [];
...
async function fetchClaimAndExitEvents(account, endBlock) {
    ...
    const eventPromise = [];
    eventPromise.push(getFilterEvents(vestFilter, lptokenStaker.interface, providerKey));
    eventPromise.push(getFilterEvents(exitFilter, lptokenStaker.interface, providerKey));
    const events = [];
    const logs = await Promise.all(eventPromise);
    logs[0].forEach((log) => {
        log.tx_type = 'entry';
...
function parseStakeUnstakeEvents(events) {
    ...
    events.forEach((event) => {
        ...
        const log = {
            ...
            lp_token_amount: div(event.lp_token_amount, BN(10).pow(poolInfos[poolId].decimals), 2),
            usd_amount: '0.00',
...
async function getPoolTransactions(account, endBlock = 'latest') {
    const result = [];
    const stakeAndUnstakeEvents = await fetchStakeAndUnstakeTransactions(account, endBlock);
    await appendEventTimestamp(stakeAndUnstakeEvents, provider);
    const stakeAndUnstakeTransactions = parseStakeUnstakeEvents(stakeAndUnstakeEvents);
