...
async function parseAdditionalData(type, hash, transactionReceipt) {
    ...
    switch (action) {
        case 'pnl':
            ...
            break;
        case 'invest':
            result = await getInvestKeyData(hash, vaultStableCoins.tokens[typeSplit[1]], transactionReceipt);
            break;
        case 'harvest':
...
async function getReceipt(type) {
    ...
    const transactionReceipt = await provider
        ...
        .catch((err) => {
        logger.error(err);
        throw new BlockChainCallError(`Get receipt of ${hash} from chain failed.`, MESSAGE_TYPES[msgLabel]);
    });
