...
const { depositEventMessage, withdrawEventMessage, summaryMessage, } = require('../../dist/discordMessage/eventMessage');
...
async function checkTvlChange(fromBlock, toBlock, depositEventResult, withdrawEventResult) {
    try {
        const controller = getLatestSystemContract(ContractNames.controller, providerKey).contract;
        ...
        const depositTotal = depositEventResult.total.pwrd.usdAmount.add(depositEventResult.total.gvt.usdAmount);
        const withdrawTotal = withdrawEventResult.total.pwrd.returnUsd.add(withdrawEventResult.total.gvt.returnUsd);
        const pnl = getLatestSystemContract(ContractNames.pnl, providerKey).contract;
        const pnlFilter = pnl.filters.LogPnLExecution();
        pnlFilter.fromBlock = fromBlock;
        pnlFilter.toBlock = toBlock;
        const pnlLogs = await getFilterEvents(pnlFilter, pnl.interface, providerKey).catch((error) => {
            logger.error(error);
            throw new ContractCallError(`Get pnl events from block ${fromBlock} to ${toBlock} failed.`, MESSAGE_TYPES.other);
        });
        let totalHarvest = BigNumber.from(0);
        pnlLogs.forEach((log) => {
            const investPnL = log.args[2];
            totalHarvest = totalHarvest.add(investPnL);
            logger.info(`checkTotalAssets pnlLogs ${log.blockNumber} investPnL ${investPnL} totalHarvest ${totalHarvest}`);
        });
        ...
        const differentRatio = different
            ...
            .div(startTvl);
        const msg = `checkTotalAssets change fromBlock ${fromBlock} toBlock ${toBlock} startTvl ${startTvl.div(USD_DECIMAL)} endTvl ${endTvl.div(USD_DECIMAL)} depositTotal ${depositTotal.div(USD_DECIMAL)} withdrawTotal ${withdrawTotal.div(USD_DECIMAL)} tvlDiff ${tvlDiff.div(USD_DECIMAL)} totalDepositAndWithdraw ${totalDepositAndWithdraw.div(USD_DECIMAL)} totalHarvest ${totalHarvest.div(USD_DECIMAL)} \ndifferent = (tvlDiff - totalDepositAndWithdraw - totalHarvest) ${different.div(USD_DECIMAL)} differentRatio ${differentRatio}`;
        logger.info(msg);
...
async function generateDepositReport(fromBlock, toBlock) {
    ...
    const latestDepositHandler = getLatestSystemContract(ContractNames.depositHandler, providerKey).contract;
    ...
    const logs = await getFilterEvents(depositFilter, latestDepositHandler.interface, providerKey).catch((error) => {
        logger.error(error);
        throw new ContractCallError(`Get deposit events from block ${fromBlock} to ${toBlock} failed.`, MESSAGE_TYPES.depositEvent);
    });
    ...
    logger.info(`Deposit events from block ${fromBlock} to ${toBlock}:\n${JSON.stringify(result)}`);
    return { total, items: result };
}
async function generateWithdrawReport(fromBlock, toBlock) {
    // generate withdraw filter
    const latestWithdrawHandler = getLatestSystemContract(ContractNames.withdrawHandler, providerKey).contract;
    ...
    const logs = await getFilterEvents(withdrawFilter, latestWithdrawHandler.interface, providerKey).catch((error) => {
        logger.error(error);
        throw new ContractCallError(`Get withdraw events from block ${fromBlock} to ${toBlock} failed.`, MESSAGE_TYPES.withdrawEvent);
    });
    ...
    logger.info(`Withdraw events from block ${fromBlock} to ${toBlock}:\n${JSON.stringify(result)}`);
    return { total, items: result };
}
async function getGTokenAsset(gtoken, blockNumber) {
    const asset = await gtoken
        ...
        .catch((error) => {
        logger.error(error);
        throw new ContractCallError(`Call totalAssets of token: ${gtoken.address} failed`);
    });
    const result = {
        originValue: asset,
...
async function generateDepositAndWithdrawReport(fromBlock, toBlock) {
    ...
    const withdrawEventResult = await generateWithdrawReport(fromBlock, toBlock);
    depositEventMessage(depositEventResult.items);
    withdrawEventMessage(withdrawEventResult.items);
    await checkTvlChange(fromBlock, toBlock, depositEventResult, withdrawEventResult);
}
...
async function generateSummaryReport(fromBlock, toBlock) {
    ...
    const depositEventResult = await generateDepositReport(fromBlock, toBlock);
    const withdrawEventResult = await generateWithdrawReport(fromBlock, toBlock);
    const latestGvt = getLatestSystemContract(ContractNames.groVault, providerKey).contract;
    const latestPWRD = getLatestSystemContract(ContractNames.powerD, providerKey).contract;
    const { originValue: originVaultValue, value: vaultTVL } = await getGTokenAsset(latestGvt, toBlock);
    const { originValue: originPwrdValue, value: pwrdTVL } = await getGTokenAsset(latestPWRD, toBlock);
    const tvl = getTVLDelta(depositEventResult.total, withdrawEventResult.total, { vault: originVaultValue, pwrd: originPwrdValue });
    const result = {
        tvl,
