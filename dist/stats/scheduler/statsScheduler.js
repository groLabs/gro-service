...
const depositWithdrawEventSchedulerSetting = getConfig('trigger_scheduler.deposit_withdraw_event', false) ||
    '*/5 * * * *';
    ...
    schedule.scheduleJob(generateStatsSchedulerSetting, async () => {
        try {
            ...
            if (failedTimes.apyGenerator >= failedAlertTimes) {
                sendAlertMessage({
                    discord: {
                        description: '[WARN] B5 - Gro stats generation failed, didn’t produce json file',
                    },
                });
            }
        }
...
function removeFile(fileNameList, removedFilePrefix) {
    ...
    fileNameList.sort((a, b) => fs.statSync(path.join(statsDir, b)).ctime.getTime() -
    ...
    schedule.scheduleJob(depositWithdrawEventSchedulerSetting, async () => {
        try {
            ...
            if (!currectBlockNumber)
                return;
            await Promise.all([
                generateDepositAndWithdrawReport(lastBlockNumber, currectBlockNumber),
            ]);
            updateLastBlockNumber(currectBlockNumber, 'lastDepositAndWithdrawBlockNumber');
            ...
            if (failedTimes.eventTrade > failedAlertTimes) {
                sendAlertMessage({
                    discord: {
                        description: "[WARN] B16 - Trade trace monitor txn failed, trade update messages didn't generate",
                    },
                });
            }
        }
    ...
    schedule.scheduleJob(eventSummarySchedulerSetting, async () => {
        try {
            ...
            if (failedTimes.eventSumary > failedAlertTimes) {
                sendAlertMessage({
                    discord: {
                        description: "[WARN] B17 - Trade summary monitor txn failed, trade summary didn't generate",
                    },
                });
            }
        }
