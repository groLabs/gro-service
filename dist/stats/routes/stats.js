...
const { getGroStatsContent, getArgentStatsContent, reloadContractsFromRegistry, } = require('../services/statsService');
 ...
 */
router.get('/gro_personal_position/', validate([
    query('address')
        .isString()
...
router.get('/update_og_airdrop', wrapAsync(async (req, res) => {
    let { network } = req.query;
    network = network || '';
    if (network.toLowerCase() !== process.env.NODE_ENV.toLowerCase()) {
        throw new ParameterError('Parameter network failed.');
...
router.get('/gro_argent_stats', wrapAsync(async (req, res) => {
    let { network } = req.query;
    network = network || '';
    if (network.toLowerCase() !== process.env.NODE_ENV.toLowerCase()) {
        throw new ParameterError('Parameter network failed.');
...
//     })
// );
router.get('/gro_price_check', wrapAsync(async (req, res) => {
    ...
    if (!isValidBlockNumber(block)) {
        ...
        throw new ParameterError(`Parameter block should be bigger than ${buoyStartBlock}.`);
    }
    const pricing = await getGroPrice(block);
    res.json({ pricing });
}));
router.get('/historical_gro_stats', wrapAsync(async (req, res) => {
    ...
    const groStats = await generateHistoricalStats(parseInt(block, 10), attr);
    res.json({ historical_gro_stats: groStats });
}));
router.get('/reload_contracts', wrapAsync(async (req, res) => {
...
router.post('/degenscore', wrapAsync(async (req, res) => {
