...
const { getAccountFailTransactionsOnAVAX, } = require('../handler/failedTransactionHandler');
const { ZERO_ADDRESS, handleError, getHandlerEvents, getContracts, getTransferHistories, getAvaxApprovalEvents, } = require('./common');
const logger = require('../statsLogger');
const rpccURL = getConfig('avalanche.rpc_url', false) ||
...
async function getTransferEvents(account, adpaterType) {
    ...
    const logs = await getTransferHistories(inTransfers, outTransfers, contractInterfaces, provider).catch((error) => {
        logger.error(error);
        handleError(error, `Get transfer logs from ${adpaterType} vault for account:${account} failed.`, account);
    });
    return logs;
}
async function calcuateUsdAmountForTransferEvents(logs, decimals) {
    ...
    logs.forEach((log) => {
        ...
        callPromise.push(vault.getPricePerShare({ blockTag: parseInt(blockNumber, 10) }));
    });
    ...
    for (let i = 0; i < result.length; i += 1) {
        ...
        const usdAmount = new BN(`${perPrice}`).multipliedBy(new BN(coinAmount));
...
async function getDepositHistory(account, token, adpaterType, decimals) {
    ...
    const logs = await getHandlerEvents(account, adpaterType, 'LogDeposit', provider, adapterHistories).catch((error) => {
        handleError(error, `Get deposit logs from ${adpaterType} vault by account:${account} failed.`, account);
    });
...
async function getWithdrawHistory(account, token, adpaterType, decimals) {
    ...
    //     needWrited = true;
    // }
    const logs = await getHandlerEvents(account, adpaterType, 'LogWithdrawal', provider, adapterHistories).catch((error) => {
        handleError(error, `Get withdraw log from ${adpaterType} valut for account:${account} failed.`, account);
    });
...
async function getVaultTokenTransferHistory(account, token, adpaterType, decimals) {
...
async function getApprovalHistory(account, contractName, decimals, token, depositEventHashs) {
    const logs = await getAvaxApprovalEvents(account, contractName, provider).catch((error) => {
        handleError(error, `Get approval filter of ${contractName} for account:${account} failed.`, account);
    });
    const result = [];
    if (!logs.length)
        return result;
...
async function singleVaultEvents(account, adpaterType, token, decimals) {
    const depositEvents = await getDepositHistory(account, token, adpaterType, decimals);
    const withdrawEvents = await getWithdrawHistory(account, token, adpaterType, decimals);
    const transferEvents = await getVaultTokenTransferHistory(account, token, adpaterType, decimals);
    ...
    // approval event
    const approvalEvents = await getApprovalHistory(account, adpaterType, decimals, token, depositEventHashs);
    return {
        depositEvents,
        withdrawEvents,
        transferEvents,
...
async function avaxPersonalStats(account) {
    ...
    const daiVaultEvents = await singleVaultEvents(account, ContractNames.AVAXDAIVault, 'DAI.e', 18);
    fullData(result, daiVaultEvents, 'dai_vault');
    const usdcVaultEvents = await singleVaultEvents(account, ContractNames.AVAXUSDCVault, 'USDC.e', 6);
    fullData(result, usdcVaultEvents, 'usdc_vault');
    const usdtVaultEvents = await singleVaultEvents(account, ContractNames.AVAXUSDTVault, 'USDT.e', 6);
    ...
    for (let i = 0; i < failTransactions.length; i += 1) {
        const { contractName, hash, blockNumber, timeStamp, to } = failTransactions[i];
        failedItems.push({
            hash,
