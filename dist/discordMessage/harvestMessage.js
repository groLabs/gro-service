...
const { MESSAGE_TYPES, MESSAGE_EMOJI, DISCORD_CHANNELS, sendMessage, sendMessageToChannel, } = require('../common/discord/discordService').default;
...
function harvestTransactionMessage(content) {
    for (let i = 0; i < content.length; i += 1) {
        ...
        const strategyName = getVaultAndStrategyLabels()[typeItems[1]].strategies[typeItems[2]]
            .name;
        ...
        const discordMessage = {
            ...
            description: `${MESSAGE_EMOJI.company} ${label} ${action} action for ${vaultName}'s ${strategyName} confirmed to chain - $${formatHarvestAmount(stableCoinDecimal, additionalData)} harvested`,
            urls: [
                {
                    label,
                    type: 'tx',
