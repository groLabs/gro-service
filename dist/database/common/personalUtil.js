...
const { getLatestContractEventFilter, getContractHistoryEventFilters, getCoinApprovalFilters, } = require('../../common/filterGenerateTool');
...
const { EVENT_TYPE, getEvents: getTransferEV, getApprovalEvents: getApprovalEV, } = require('../../common/logFilter');
...
const getApprovalEvents2 = async (account, fromBlock, toBlock) => {
    try {
        const logApprovals = await getCoinApprovalFilters('default', fromBlock, toBlock, account);
        const logPromises = [];
        for (let i = 0; i < logApprovals.length; i += 1) {
            const approvalEvent = logApprovals[i];
            logPromises.push(getFilterEvents(approvalEvent.filter, approvalEvent.interface, 'default'));
        }
