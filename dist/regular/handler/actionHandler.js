const { getController, getInsurance, getLifeguard, getVaults, getVaultAndStrategyLabels, getCurveVault, getBuoy, } = require('../../dist/contract/allContracts');
const { pendingTransactions } = require('../../common/storage');
const { addPendingTransaction, } = require('../../dist/common/pendingTransaction');
const { ContractSendError, ContractCallError, } = require('../../dist/common/error');
...
const { withdrawMessage, distributeMessage, } = require('../../dist/discordMessage/distributeCurveMessage');
...
async function adapterInvest(blockNumber, isInvested, vault, providerKey, walletKey) {
    ...
    for (let i = 0; i < pendingKeys.length; i += 1) {
        if (pendingKeys[i].startWith(`harvest-${vault.address}`)) {
            ...
            const result = `Already has pending harvest for ${vaultName}:${vault.address} transaction: ${pendingTransactions.get(`invest-${vault.address}`).hash}`;
            getWalletNonceManager(providerKey, walletKey).setTransactionCount(currentNonce);
            logger.info(result);
        }
    }
...
async function distributeCurveVault(blockNumber, providerKey, walletKey, amount, delta) {
    ...
    addPendingTransaction('withdrawToAdapter', {
        blockNumber,
        providerKey,
        walletKey,
        reSendTimes: 0,
    ...
    addPendingTransaction('distributeCurveAssets', {
        blockNumber,
        providerKey,
        walletKey,
        reSendTimes: 0,
