...
const { distributeCurveVaultTriggerMessage, } = require('../../discordMessage/distributeCurveMessage');
...
async function isEmergencyState(messageType, providerKey, walletKey) {
    ...
    const emergencyState = await controller.emergencyState().catch((error) => {
        logger.error(error);
        throw new ContractCallError("Get system's emergency state failed", messageType);
    });
...
async function adapterInvestTrigger(vault) {
    ...
    if (pendingTransactions.get(`invest-${vault.address}`)) {
        const result = `Already has pending invest for ${vaultName}:${vault.address} transaction: ${pendingTransactions.get(`invest-${vault.address}`).hash}`;
        logger.info(result);
    ...
    const investTriggerResult = await vault.investTrigger().catch((error) => {
        logger.error(error);
        throw new ContractCallError(`Call investTrigger of ${vaultName} : ${vault.address} to check if the adapter need investment failed`, MESSAGE_TYPES.investTrigger);
    });
    logger.info(`${vaultName} : ${vault.address} invest trigger: ${investTriggerResult}`);
...
async function curveInvestTrigger(vault, lifeguard) {
    ...
    const investTriggerResult = await lifeguard
        ...
        .catch((error) => {
        logger.error(error);
        throw new ContractCallError(`Call investToCurveVaultTrigger of ${vaultName} : ${vault.address} to check if the lifeguard need investment failed`, MESSAGE_TYPES.investTrigger);
    });
    logger.info(`${vaultName} : ${vault.address} invest trigger: ${investTriggerResult}`);
}
async function sortStrategyByLastHarvested(vaults) {
    if (vaults.length === 0) {
        ...
        throw new ContractCallError('Not found any vaults', MESSAGE_TYPES.harvestTrigger);
    }
    ...
    for (let i = 0; i < vaults.length; i += 1) {
        ...
        const strategyArray = getVaultAndStrategyLabels()[adapterAddress].strategies;
        ...
        for (let j = 0; j < strategyLength; j += 1) {
            ...
            const triggerResult = await vault.strategyHarvestTrigger(j, callCost);
            logger.info(`triggerResult ${triggerResult}`);
            // eslint-disable-next-line no-await-in-loop
            const strategyParam = await yearnVault.strategies(strategyArray[j].address);
            logger.info(`strategyParam ${strategyArray[j].address} ${strategyParam}`);
            // eslint-disable-next-line no-await-in-loop
            const estimatedTotalAssets = await strategyArray[j].strategy.estimatedTotalAssets();
            logger.info(`strategy estimated total assets ${estimatedTotalAssets} totalDebt ${strategyParam.totalDebt} expectedReturn ${estimatedTotalAssets.sub(strategyParam.totalDebt)} `);
    ...
    const sorted = strategiesStatus.sort((a, b) => a.lastHarvest - b.lastHarvest);
    return sorted;
}
async function investTrigger(providerKey, walletKey) {
    // emergency check
    const isEmergency = await isEmergencyState(MESSAGE_TYPES.investTrigger, providerKey, walletKey);
    if (isEmergency) {
        logger.info('System is in emergency state, invest action will be paused.');
    ...
    triggerPromises.push(curveInvestTrigger(vaults[lastVaultIndex], getLifeguard(providerKey, walletKey)));
    ...
    for (let i = 0; i < strategies.length; i += 1) {
        ...
        logger.info(`strategy ${vaultIndex}, ${strategyIndex}, ${lastHarvest}, ${result[vaultIndex]}`);
        // TODO: skip xpool check at the beginning
        if (strategies[i].trigger && vaultIndex < 3) {
            if (result[vaultIndex]) {
                logger.info(`vault trigger check ${needInvestIndex} ${result[vaultIndex]}`);
...
async function harvestOneTrigger(providerKey, walletKey) {
    // emergency check
    const isEmergency = await isEmergencyState(MESSAGE_TYPES.harvestTrigger, providerKey, walletKey);
    if (isEmergency) {
        logger.info('System is in emergency state, harvest action will be paused.');
        return NONEED_TRIGGER;
    }
    const vaults = getVaults(providerKey, walletKey);
    const strategies = await sortStrategyByLastHarvested(vaults);
    ...
    logger.info(`gasPrice ${gasPrice}`);
    for (let i = 0; i < strategies.length; i += 1) {
        const { vaultIndex, strategyIndex, trigger, totalDebt, estimatedTotalAssets, } = strategies[i];
        logger.info(`harvestOneTrigger ${vaultIndex} ${strategyIndex} ${trigger}`);
        ...
        if (vaultIndex === 3) {
            ...
            if (callCost.mul(BigNumber.from(10000)).lte(expectedReturn)) {
                logger.info(`curve strategy is ready to harvest ${callCost.mul(BigNumber.from(10000))} < ${expectedReturn}`);
                return {
                    needCall: true,
                    params: {
                        vault: vaults[vaultIndex],
...
async function rebalanceTrigger(providerKey, walletKey) {
    // emergency check
    const isEmergency = await isEmergencyState(MESSAGE_TYPES.rebalanceTrigger, providerKey, walletKey);
    if (isEmergency) {
        logger.info('System is in emergency state, rebalance action will be paused.');
        return NONEED_TRIGGER;
    }
    if (pendingTransactions.get('rebalance')) {
        const result = `Already has pending rebalance transaction: ${pendingTransactions.get('rebalance').hash}`;
        logger.info(result);
        throw new PendingTransactionError(result, MESSAGE_TYPES.rebalanceTrigger);
    }
    const needRebalance = await getInsurance(providerKey, walletKey)
        .rebalanceTrigger()
        .catch((error) => {
        logger.error(error);
        throw new ContractCallError('RebalanceTrigger call failed', MESSAGE_TYPES.rebalanceTrigger);
    });
...
async function distributeCurveVaultTrigger(providerKey, walletKey) {
    ...
    if (pendingTransactions.get('withdrawToAdapter') ||
        pendingTransactions.get('distributeCurveAssets')) {
        const result = `Already has pending curve-exposure transaction: ${pendingTransactions.get('withdrawToAdapter').hash ||
            pendingTransactions.get('distributeCurveAssets').hash}`;
        logger.info(result);
        throw new PendingTransactionError(result, MESSAGE_TYPES.distributeCurveVault);
    }
    ...
    logger.info(`curve exposure: ${curveExposure} curve target: ${curveTarget}`);
    ...
    if (curveExposure.gt(curveTarget)) {
        ...
        const lpAmountNeedDistribute = await bouy3pool.usdToLp(usdAmountNeedDistribute);
        logger.info(`usdAmountNeedDistribute ${usdAmountNeedDistribute} lpAmountNeedDistribute: ${lpAmountNeedDistribute}`);
