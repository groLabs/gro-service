...
const { PendingTransactionError, ContractCallError, } = require('../../common/error');
...
async function distributeCurveVaultTrigger(providerKey, walletKey) {
    ...
    if (pendingTransactions.get('curve-exposure')) {
        const result = `Already has pending curve-exposure transaction: ${pendingTransactions.get('curve-exposure').hash}`;
        logger.info(result);
