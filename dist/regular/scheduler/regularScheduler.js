...
const { syncManagerNonce, checkAccountsBalance, getCurrentBlockNumber, } = require('../../dist/common/chainUtil');
const { checkServerHealth } = require('../../dist/common/checkBotHealth');
const { checkPendingTransactions, syncPendingTransactions, } = require('../../dist/common/pendingTransaction');
const { pendingTransactions } = require('../../common/storage');
const { sendErrorMessageToLogChannel, } = require('../../dist/common/discord/discordService');
const { pendingTransactionResend } = require('../../dist/gasPrice/transaction');
const { investTrigger, rebalanceTrigger, harvestOneTrigger, distributeCurveVaultTrigger, } = require('../handler/triggerHandler');
const { invest, rebalance, harvest, curveInvest, priceSafetyCheck, distributeCurveVault, } = require('../handler/actionHandler');
const { getVaults, getStrategyLength, } = require('../../dist/contract/allContracts');
const { getConfig } = require('../../dist/common/configUtil');
const { investTransactionMessage, } = require('../../dist/discordMessage/investMessage');
const { pnlTransactionMessage, } = require('../../dist/discordMessage/pnlMessage');
const { rebalanceTransactionMessage, } = require('../../dist/discordMessage/rebalanceMessage');
const { harvestTransactionMessage, } = require('../../dist/discordMessage/harvestMessage');
const { updatePriceTransactionMessage, } = require('../../dist/discordMessage/otherMessage');
const { distributeCurveVaultTransactionMessage, } = require('../../dist/discordMessage/distributeCurveMessage');
...
const investTriggerSchedulerSetting = getConfig('trigger_scheduler.invest', false) || '0 * * * *';
const harvestTriggerSchedulerSetting = getConfig('trigger_scheduler.harvest', false) || '15 * * * *';
const rebalanceTriggerSchedulerSetting = getConfig('trigger_scheduler.rebalance', false) || '45 * * * *';
const safetyCheckSetting = getConfig('trigger_scheduler.safety_check', false) || '*/5 * * * *';
    ...
    schedule.scheduleJob(safetyCheckSetting, async () => {
        ...
        try {
            ...
            if (failedTimes.safetyCheck >= 2) {
                sendAlertMessage({
                    discord: {
                        description: '[WARN] B8 - Buoy’s safetycheck txn failed, price safety check didn’t complete',
                    },
                });
            }
        }
...
async function resendPendingTransaction(types) {
    for (let i = 0; i < types.length; i += 1) {
        ...
        const pendingTimeSetting = longPendingTransactionSetting[methodName] || 120000;
        if (pendingTimes > pendingTimeSetting) {
            logger.info(`Transaction[${type}] ${hash} already pending ${pendingTimes / 1000} seconds, more than setting ${pendingTimeSetting / 1000} seconds, will resend this transaction`);
...
async function checkLongPendingTransactions() {
    ...
    logger.info(`Long Pending transaction : size ${longPendingTransactions.length}, transactions ${longPendingTransactions}`);
    ...
    schedule.scheduleJob(investTriggerSchedulerSetting, async () => {
        try {
            ...
            if (investTriggers.needCall && investTriggers.params < 3) {
                await invest(currectBlockNumber, investTriggers.params, providerKey, walletKey);
            }
            if (investTriggers.needCall && investTriggers.params === 3) {
            ...
            const discordMessage = {
                description: "[WARN] B1 - InvestTrigger | Invest txn failed, InvestTrigger action didn't complate",
            };
            sendAlertMessage({
...
function rebalanceTriggerScheduler() {
    ...
    schedule.scheduleJob(rebalanceTriggerSchedulerSetting, async () => {
        try {
            ...
            const triggerResult = await rebalanceTrigger(providerKey, walletKey);
            ...
            sendErrorMessageToLogChannel(error);
            const discordMessage = {
                description: "[WARN] B3 - RebalanceTrigger | Rebalance txn failed, RebalanceTrigger action didn't complate",
            };
            sendAlertMessage({
                discord: discordMessage,
            });
    ...
    schedule.scheduleJob(rebalanceTriggerSchedulerSetting, async () => {
        try {
            ...
            if (!currectBlockNumber)
                return;
            await syncManagerNonce(providerKey, walletKey);
            await distributeCurveVault(currectBlockNumber, providerKey, walletKey, triggerResult.params.amount, triggerResult.params.delta);
        }
        catch (error) {
            sendErrorMessageToLogChannel(error);
            const discordMessage = {
...
function harvestTriggerScheduler() {
    ...
    schedule.scheduleJob(harvestTriggerSchedulerSetting, async () => {
        try {
            ...
            const triggerResult = await harvestOneTrigger(providerKey, walletKey);
            if (!triggerResult.needCall)
                return;
            const currectBlockNumber = await getCurrentBlockNumber(providerKey);
            if (!currectBlockNumber)
                return;
            await syncManagerNonce(providerKey, walletKey);
            await harvest(currectBlockNumber, triggerResult.params, providerKey, walletKey);
        }
        catch (error) {
            sendErrorMessageToLogChannel(error);
            const discordMessage = {
                description: "[WARN] B2 -  HarvestTrigger | Harvest txn failed, HarvestTrigger action didn't complate",
            };
            sendAlertMessage({
                discord: discordMessage,
            });
