        ...
        logs.forEach((log) => {
            const dai_amount =
                side === Transfer.DEPOSIT
                    ? parseAmount(log.args[4][0], Base.D18) // LogNewDeposit.tokens[0]
                    : side === Transfer.WITHDRAWAL
                        ? -parseAmount(log.args[8][0], Base.D18) // LogNewWithdrawal.tokenAmounts[0]
                        : 0;

            const usdc_amount =
                side === Transfer.DEPOSIT
                    ? parseAmount(log.args[4][1], Base.D6) // LogNewDeposit.tokens[1]
                    : side === Transfer.WITHDRAWAL
                        ? -parseAmount(log.args[8][1], Base.D6) // LogNewWithdrawal.tokenAmounts[1]
                        : 0;

            const usdt_amount =
                side === Transfer.DEPOSIT
                    ? parseAmount(log.args[4][2], Base.D6) // LogNewDeposit.tokens[2]
                    : side === Transfer.WITHDRAWAL
                        ? -parseAmount(log.args[8][2], Base.D6) // LogNewWithdrawal.tokenAmounts[2]
                        : 0;

            const usd_deduct =
                side === Transfer.WITHDRAWAL
                    ? -parseAmount(log.args[5], Base.D18) // LogNewWithdrawal.deductUsd
                    : 0;

            const lp_amount =
                side === Transfer.WITHDRAWAL
                    ? -parseAmount(log.args[7], Base.D18) // LogNewWithdrawal.lpAmount
                    : 0;
            ...
            const usd_return =
                side === Transfer.WITHDRAWAL
                    ...
                    : side === Transfer.TRANSFER_PWRD_OUT
                        ? -parseAmount(log.args[2], Base.D18) // Transfer.value
                        : 0;
            ...
            const usdc_e_amount =
                side === Transfer.DEPOSIT_USDCe
                    ...
                    ? parseAmount(log.args[2], Base.D6) // LogDeposit.shares, Transfer.value
                    : side === Transfer.WITHDRAWAL_USDCe
                        || side === Transfer.TRANSFER_USDCe_OUT
                        ? -parseAmount(log.args[2], Base.D6) // LogWithdrawal.shares, Transfer.value
                        : 0;
            ...
            const dai_e_amount =
                side === Transfer.DEPOSIT_DAIe
                    ...
                    ? parseAmount(log.args[2], Base.D18) // LogDeposit.shares, Transfer.value
                    : side === Transfer.WITHDRAWAL_DAIe
                        || side === Transfer.TRANSFER_DAIe_OUT
                        ? -parseAmount(log.args[2], Base.D18) // LogWithdrawal.shares, Transfer.value
                        : 0;

            result.push({
                block_number: log.blockNumber,
                tx_hash: log.transactionHash,
                network_id: getNetwork(network).id,
