...
const loadEthBlocks2 = async (func) => {
    try {
        ...
        if (numBlocks > 0) {
            logger.info(
                `**DB CACHE: Processing ${numBlocks} block${isPlural(
                    numBlocks
                )} from ${
                    func === 'loadUserTransfers' ? 'transfers' : 'approvals'
                }...`
            );
            ...
            logger.info(
                `**DB CACHE: ${numBlocks} block${isPlural(
                    numBlocks
                )} added into ETH_BLOCKS`
            );
        } else {
            logger.info(
                `**DB CACHE: No blocks to be added from ${
                    func === 'loadUserTransfers' ? 'transfers' : 'approvals'
                }`
            );
        }
...
const loadUserNetReturns2 = async (fromDate, toDate, account) => {
    try {
        ...
        for (const date of dates) {
            ...
            const result = await query('insert_cache_user_net_returns.sql', [
                account,
            ]);
            if (result.status === QUERY_ERROR) return false;
            const numResults = result.rowCount;
            let msg = `**DB CACHE: ${numResults} record${isPlural(
                numResults
            )} added into `;
            msg += `USER_NET_RETURNS for date ${moment(date).format(
                'DD/MM/YYYY'
            )}`;
            logger.info(msg);
        }
        ...
        handleErr(
            `personalHandler->loadUserNetReturns() [from: ${fromDate}, to: ${toDate}]`,
            err
        );
    }
};
...
const loadUserBalances2 = async (fromDate, toDate, account) => {
    try {
        ...
        const dates = generateDateRange(fromDate, toDate);
        logger.info(
            `**DB CACHE: Processing ${users.rowCount} user balance${isPlural(
                users.rowCount
            )}...`
        );
        for (const date of dates) {
            ...
            for (const user of users.rows) {
                ...
                const gvtValue = parseAmount(
                    await getGvt().getAssets(account, blockTag),
                    'USD'
                );
                const pwrdValue = parseAmount(
                    await getPwrd().getAssets(account, blockTag),
                    'USD'
                );
            ...
            let msg = `**DB CACHE: ${rowCount} record${isPlural(
                rowCount
            )} added into `;
            msg += `USER_BALANCES for date ${moment(date).format(
                'DD/MM/YYYY'
            )}`;
            logger.info(msg);
        }
        ...
        handleErr(
            `personalHandler->loadUserBalances() [from: ${fromDate}, to: ${toDate}]`,
            err
        );
        return false;
    }
...
const loadUserApprovals2 = async (account) => {
    try {
        ...
        if (await loadEthBlocks2('loadUserApprovals')) {
            ...
            const res = await query('insert_cache_user_approvals.sql', [
                account,
            ]);
            if (res.status === QUERY_ERROR) return false;
            const numTransfers = res.rowCount;
            logger.info(
                `**DB CACHE: ${numTransfers} record${isPlural(
                    numTransfers
                )} added into USER_APPROVALS`
            );
            return true;
        } else {
            return false;
...
const loadUserTransfers2 = async (account) => {
    try {
        ...
        if (await loadEthBlocks2('loadUserTransfers')) {
            // Load deposits & withdrawals from temporary tables into USER_TRANSFERS
            const res = await query('insert_cache_user_transfers.sql', [
                account,
            ]);
            if (res.status === QUERY_ERROR) return false;
            const numTransfers = res.rowCount;
...
const loadTempUserApprovals = async (fromBlock, toBlock, account) => {
    try {
        ...
        logger.info(
            `**DB CACHE: Processing ${logs.length} approval event${isPlural(
                logs.length
            )}...`
        );
        ...
        handleErr(
            `personalHandler->loadTmpUserApprovals() [blocks: ${fromBlock} to: ${toBlock}]`,
            err
        );
        return false;
    }
...
const loadTempUserTransfers = async (
    ...
    try {
        const logs = await getTransferEvents2(
            side,
            fromBlock,
            toBlock,
            account
        ...
        if (logs) {
            ...
            if (logs.length > 0) {
                ...
                if (!res) return false;
                logger.info(
                    `**DB CACHE: ${rows} ${transferType(side)}${isPlural(
                        rows
                    )} added into ${
                        isDeposit(side)
            ...
            logger.info(
                `**DB CACHE: 0 ${transferType(side)}} added into ${
                    isDeposit(side)
                        ? 'CACHE_TMP_USER_DEPOSITS'
                        : 'CACHE_TMP_USER_WITHDRAWALS'
                }`
...
const preloadCache = async (account) => {
    try {
        ...
        if (
            tmpApprovals.status === QUERY_ERROR ||
        ...
        if (!_fromDate.rows[0].max_balance_date) {
            ...
            fromDate = moment.unix(timestamp).utc();
        ...
        const toDate = moment.utc().format('DD/MM/YYYY');
...
const loadCache = async (account) => {
    try {
        ...
        if (fromBlock > 0) {
            ...
            handleErr(
                `personalHandler->reload() Error with parameters: ${params}`,
                null
            );
        }
    } catch (err) {
