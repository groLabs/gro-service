...
const { investTrigger: callInvestTrigger } = require('./contract/insurance')
const { getInsurance, getPnl } = require('./contract/controller')
const { pendingTransactions } = require('../common/storage')
const { getVaults } = require('./contract/controller')
const { strategiesLength, strategyHarvestTrigger } = require('./contract/vault')
...
const checkVaultStrategyHarvest = async function (vault, vaultIndex) {
  ...
  for (let i = 0; i < strategyLength; i++) {
    ...
    const triggerResponse = await strategyHarvestTrigger(vault, i, callCost)
