...
async function setBorrowLimit(vault) {
    try {
        ...
        const { safeAvaxAmount: safeAvax, isOverBorrowed } =
            await getSafeAvaxAmount(ahStrategy, {
                blockTag: latestBlock.number,
            });
        let newBorrowLimit = AH_CREDIT;
        if (safeAvax.gte(AH_CREDIT)) {
            const { safeAvaxAmount: safeAvaxOneMinAgo } =
                await getSafeAvaxAmount(ahStrategy, {
        ...
        if (safeAvax.lt(AH_CREDIT)) {
            ...
            //)
            logger.info(
                `whether need update the borrowlimit: diff ${diff.gt(
