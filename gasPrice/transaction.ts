...
import { getAlchemyPriorityPrice, getAlchemy24HoursLowestFee } from './priceManager';
...
async function wrapSendTransaction(contract, methodName, params = []) {
    ...
    logger.info(
        `send tx maxPriorityFeePerGas ${maxPriorityFeePerGas} maxBaseFeePerGas ${baseFeePerGas} maxFeePerGas ${maxFeePerGas}`
    );
