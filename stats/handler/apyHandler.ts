...
import { BlockChainCallError } from '../../common/error';
...
function calculatePriceDiff(factorStart, factorEnd) {
    ...
    if (startPrice.toString() === '0') {
        throw new BlockChainCallError(
            'calculatePriceDiff failed: startPrice is 0.'
        );
    }
...
async function calcApyByPeriod(
    startDate,
    currentGvtFactor,
    currentPwrdFactor,
    multiplier
    ...
    logger.info(
        `calcApyByPeriod block ${startBlock.block} ${currentGvtFactor} ${currentPwrdFactor} factor gvt ${startGvtFactor} ${gvtApy} pwrd ${startPwrdFactor} ${pwrdApy}`
    );
    return {
        pwrd: pwrdApy,
        gvt: gvtApy,
...
async function getSystemApy(latestBlock, provider) {
    ...
    if (oneMonthAgo.isAfter(launchDate)) {
        apyMonthly = await calcApyByPeriod(
            oneMonthAgo,
            gvtFactorUtcToday,
            pwrdFactorUtcToday,
            MONTHS_IN_YEAR
    ...
    if (oneWeekAgo.isAfter(launchDate)) {
        apyWeekly = await calcApyByPeriod(
            oneWeekAgo,
            gvtFactorUtcToday,
            pwrdFactorUtcToday,
            WEEKS_IN_YEAR
