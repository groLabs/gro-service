...
const { execActions } = require("./actionHandler");
const { callTriggers } = require("./triggerHandler");
...
class BlockWorker {
  ...
  async checkPendingTransactions() {
    ...
    Array.from(transactionTypes).forEach(async (type) => {
      ...
      const transactionReceipt = await provider.getTransactionReceipt(hash).catch((err) => {
        logger.error(err);
  ...
  async syncNounce() {
    ...
    const transactionCountInChain = await nonceManager.getTransactionCount().catch((error) => {
      logger.error(error);
      return -1;
    });
