...
const { getInsurance, getPnl, getVaults, getStrategyLength } = require("../contract/allContracts");
const { pendingTransactions } = require("../common/storage");
const logger = require("../common/logger");
const config = require("config");
const { exceptions } = require("../common/logger");
...
const checkVaultStrategyHarvest = function (vault, vaultIndex, strategyLength) {
  ...
  for (let i = 0; i < strategyLength; i++) {
    ...
    promises.push(
      vault.strategyHarvestTrigger(i, callCost).then((resolve, reject) => {
        logger.info(`success ${vault.address} ${i} ${resolve}`);
        return { vault: vault, strategyIndex: i, callCost, triggerResponse: resolve };
      })
    );
  }
  return promises;
